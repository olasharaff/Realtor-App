...
import { FaChair, FaParking, FaTrash } from "react-icons/fa";
import { MdEdit } from "react-icons/md";


export default function SpecialListing({ listing, id, onDelete, onEdit }) {
  return (
    <li className="relative bg-white flex flex-col justify-between items-center shadow-xl hover:shadow-lg rounded-md overflow-hidden transition-shadow duration-150 m-[10px] ">
      <Link className="contents" to={`/category/${listing.type}/${id}`}>
        ...
        />
        <p
          className="absolute top-2 left-2 bg-[#0b801f] text-white text-[10px] font-medium rounded-xl px-2 py-1 shadow-lg"
          fromNow
        >
          Rent Special
        </p>
